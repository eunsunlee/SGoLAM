...
docker run -it --rm --ipc="host" -v $DATA_DIR:/multion-chal-starter/data/ -v $ORIG_DATA_DIR:/home/jhkim/Datasets/mp3d_habitat/mp3d/ --runtime=nvidia esteshills/multion_test:tagname bash

# Full Docker command if symlink is not used: comment the line above and use this one instead
# docker run -it --rm --ipc="host" -v $DATA_DIR:/multion-chal-starter/data/ --runtime=nvidia esteshills/multion_test:tagname bash
